...
class SuperRPC {
    ...
    callId = 0;
    proxyObjectRegistry = new proxy_object_registry_1.ProxyObjectRegistry();
    ...
    sendRemoteDescriptors(replyChannel = this.channel) {
        this.sendSyncIfPossible({
            action: 'descriptors',
            objects: this.getLocalDescriptors(this.hostObjectRegistry, rpc_descriptor_types_1.processObjectDescriptor),
            functions: this.getLocalDescriptors(this.hostFunctionRegistry, rpc_descriptor_types_1.processFunctionDescriptor),
                ...
                case 'method_call': {
                    ...
                    if (!descriptor && !target[msg.prop]) {
                        ...
                        if (eventName && (addOrRemove === 'add' || addOrRemove === 'remove') &&
                            ...
                            const evtDescriptor = (0, rpc_descriptor_types_1.getEventDescriptor)(descriptor, eventName);
    ...
    createVoidProxyFunction(objId, func, action, replyChannel) {
        ...
        const fn = function (...args) {
            if (fn[proxy_object_registry_1.rpc_disposed])
...
//# sourceMappingURL=super-rpc.js.map